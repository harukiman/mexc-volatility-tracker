...
**スコア30-50のシグナルは取引コスト (0.04%) を超えるエッジを持たない (-Sharpe)。ただしスコア55+のサブセット (n=533) ではSharpe 7.28、累積 83.2% と強力なアルファを確認。。** 実運用においてはスコア55以上のシグナルのみを採用し、30-50は除外することを強く推奨する。FR実データ統合により更なる改善が期待できる。。
...
| 40-50 | 35,785 | 46.0% | -0.048% | -5.35 | -98.2% |
| 50-55 | 17,360 | 45.5% | -0.037% | -1.32 | -79.8% |
| 55+ | 533 | 46.9% | 0.135% | 7.28 | -21.3% |
...
- **55+バケット**: **唯一プラスのバケット**。LONGシグナルで `uptrend + accel + liq+` の3条件同時成立。Sharpe 7.28、累積 83.2%

**重要発見**: v5スコアにはスコア55を境に「エッジ有り」「エッジなし」の明確な分岐がある。スコア55+に限定すると n=533 と少ないが、投資委員会レベルで統計的に意味のある結果。

## 4.3 タグ別有効性分析 (LONG, score >= 30)

| タグ | n | 勝率 | avg Raw PnL | avg Net PnL |
|------|---|------|-------------|-------------|
| illiq | 668 | 44.5% | 0.0947% | 0.0547% |
| accel | 2,625 | 46.7% | 0.0869% | 0.0469% |
| uptrend | 31,644 | 45.4% | 0.0041% | -0.0359% |
| liq+ | 33,708 | 45.3% | 0.0037% | -0.0363% |
| weak-up | 17,533 | 44.9% | -0.0016% | -0.0416% |
...
**スコア30-50のシグナルは取引コスト (0.04%) を超えるエッジを持たない (-Sharpe)。ただしスコア55+のサブセット (n=533) ではSharpe 7.28、累積 83.2% と強力なアルファを確認。**

| バケット | n | Sharpe | 推奨 | 根拠 |
|---------|---|--------|------|------|
| スコア30-40 | 36,376 | -4.81 | **非推奨** | raw PnL≈0、コストで完全侵食 |
| スコア40-50 | 35,785 | -5.35 | **非推奨** | 最多トレード数、最悪Sharpe |
| スコア50-55 | 17,360 | -1.32 | **非推奨** | 僅かに改善するが依然マイナス |
| スコア55+ | 533 | 7.28 | **推奨候補** | 正のエッジあり。FR実データ追加で更改善期待 |
...
実運用においてはスコア55以上のシグナルのみを採用し、30-50は除外することを強く推奨する。FR実データ統合により更なる改善が期待できる。

**スコア55+に絞った場合の理論的シグナル頻度**: 年間 533 トレード (月平均約 44 件)。低頻度のため資金効率は低いが、ノイズが少ない。
...
*レポート生成: 2026-05-31 17:29:39 JST*
